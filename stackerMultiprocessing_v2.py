...
class Stacker:
    ...
    def start_queue(self, process, *extra_args):
        ...
        new_process = Process(target=process, args=(child_connection, *extra_args))
        new_process.start()

        running = True
        while running:
            image_array = parent_connection.recv()
            if image_array is None:
                running = False
            else:
                # Tomo el tiempo proceso para la imagen
                start_time = time.time()
                self.process_im_array(image_array)
    ...
    def get_images_from_path(self, conn, path):
        ...
        for file in os.listdir(path):
            ...
            image_array = np.asarray(img)  # Tomo la matriz
            img.close()

            log("To Array: %f" % (time.time() - start_time))

            conn.send(image_array)
        conn.send(None)
        conn.close()
